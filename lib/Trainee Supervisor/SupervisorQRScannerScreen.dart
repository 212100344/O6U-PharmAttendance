// lib/Trainee Supervisor/SupervisorQRScannerScreen.dart
...
class SupervisorQRScannerScreen extends StatefulWidget {
  const SupervisorQRScannerScreen({Key? key}) : super(key: key);

  @override
  _SupervisorQRScannerScreenState createState() =>
      _SupervisorQRScannerScreenState();
}

class _SupervisorQRScannerScreenState extends State<SupervisorQRScannerScreen> {
  ...
  void _foundBarcode(BarcodeCapture barcode) async {
    ...
    if (code.isEmpty) {
      print('Failed to scan Barcode'); //keep it!
      return;
    }
    ...
    try { // Added a try-catch block here
      final qrData = _parseQrCodeData(code); // Parse data to Map
      final reportType = qrData['reportType'];
      final studentId = qrData['studentId'];
      print("Parsed Report Type: $reportType"); // Debug: Parsed report type
      print("Parsed Student ID: $studentId");
      if (reportType != "single") { // Check the reportType directly
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                  "Invalid report type. Supervisors can only view single-round reports."),
              backgroundColor: Colors.red,
            ),
          );
        }
        _screenOpened = false; // reset flag
        return;
      }
      ...
      _screenOpened = false; // reset on error
    }
  }

  // Fetch round details for report type "single".
  Future<Map<String, dynamic>?> _fetchRoundDetails(String roundId) async {
    try {
      ...
      print("Round data fetched: $response");
      return response; // Return Map or null
    } catch (e) {
