// lib/Admin/Evaluation/AllRoundsReportScreenAdmin.dart
...
import '../pdf_service_admin.dart';

class AllRoundsReportScreenAdmin extends StatefulWidget {
  final String studentId;

  const AllRoundsReportScreenAdmin({Key? key, required this.studentId})
      : super(key: key);

  @override
  State<AllRoundsReportScreenAdmin> createState() =>
      _AllRoundsReportScreenAdminState();
}

class _AllRoundsReportScreenAdminState extends State<AllRoundsReportScreenAdmin> {
  bool isLoading = true;
  Map<String, dynamic>? studentData;
  List<Map<String, dynamic>> roundsData = [];
  int _cumulativeAttendancePercentage = 0;

  @override
  void initState() {
    super.initState();
    print(
        "AllRoundsReportScreenAdmin: studentId = ${widget.studentId}"); // Debug print
    _fetchReportData();
  }
  ...
  Future<void> _fetchReportData() async {
    ...
    try {
      ...
      print("Student Response: $studentResponse"); // Debug print
      ...
      final enrolledRoundsResponse = await Supabase.instance.client
          .from('student_rounds')
          .select(
          'round_id, rounds(id, name, start_date, end_date, leader_id, location)')
          .eq('student_id', widget.studentId);
      print("Enrolled Rounds Response: $enrolledRoundsResponse"); // Debug print

      List<Map<String, dynamic>> enrolledRounds =
      List<Map<String, dynamic>>.from(enrolledRoundsResponse);
      ...
      for (var enrollment in enrolledRounds) {
        ...
        round['allDaysExcluded'] = allDaysExcluded;
        round['supervisorName'] = supervisorName;

        roundsData.add(round);
      }

      _cumulativeAttendancePercentage = totalExpectedDays > 0
          ? ((totalAttendedDays / totalExpectedDays) * 100).round()
          : 0;
      ...
      print("Error fetching report data: $e"); // Keep for debugging
      setState(() {
        isLoading = false;
      });
    }
  ...
  Future<void> _generatePdf() async {
    ...
    // Construct QR code data.  Indicates an "all rounds" report.
    final qrCodeData = "reportType:all|studentId:${widget.studentId}";

    final pdfData = await PdfServiceAdmin.generateAllRoundsReport(
      student: student,
      rounds: rounds, // Pass the list here
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow(
              Icons.all_inclusive, // A suitable icon
              "Cumulative Attendance (All Rounds)",
              "$_cumulativeAttendancePercentage%",
              valueColor:  _getAttendanceColor(_cumulativeAttendancePercentage),
            ),
          ],
        ),
      ),
      ...
      child: Column(
        children: [
          CircularPercentIndicator(
            ...
            center: Text(
              "$_cumulativeAttendancePercentage%", // show percentage
              style: GoogleFonts.poppins(
                fontSize: 28,
            ...
            progressColor: Colors.greenAccent, // Different color for distinction
  ...
  Widget _buildRoundCard(Map<String, dynamic> round) {
    final allDaysExcluded = round['allDaysExcluded'] == true;  //check
    return Card(
            ...
            Row(
              ...
              children: [
                Text(
                  ...
                  // allDaysExcluded ? "Attendance: N/A": "Attendance: ${round['attendedDays']} / ${round['expectedDays']} days", // NEW
                  style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w500),
                ),
                CircularPercentIndicator(
                  radius: 40,
                  lineWidth: 8,
                  percent: allDaysExcluded? 1.0 : round['attendancePercentage'] / 100, // Change here,
                  center: Text(
                    "${round['attendancePercentage']}%", // display percentage
                    style: GoogleFonts.poppins(
                      fontSize: 12,
                      fontWeight: FontWeight.w600,
